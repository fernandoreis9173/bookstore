...
import { LastJob } from 'src/app/models/lastJob';
import { LastJobService } from 'src/app/services/lastJob.service';

@Component({
  selector: 'app-lastJob',
  templateUrl: './lastJob.component.html',
  styleUrls: ['./lastJob.component.scss'],
})
export class LastJobComponent {
  tipoTela: number = 1; // 1 listagem, 2 cadastro, 3 edição
  tableListLastJob: Array<LastJob>;
  id: string;
  ...
  ngOnInit() {
    ...
    this.LastJobForm = this.formBuilder.group({
      id: [''],
      ultimoEmprego: ['', [Validators.required]],
      endereco: ['', [Validators.required]],
      cidade: ['', [Validators.required]],
      telefone: ['', [Validators.required]],
      celular: ['', [Validators.required]],
      funcao: ['', [Validators.required]],
      tempoDeTrabalho: ['', [Validators.required]],
      assinouCarteira: ['', [Validators.required]],
      dataEntrada: ['', [Validators.required]],
      dataSaida: ['', [Validators.required]],
      motivoSaida: ['', [Validators.required]],
    });
  }
  ...
  onFundamentalChange(event: Event) {
    ...
    if (this.isSerieFundamentalDisabled) {
      this.LastJobForm.get('serieFundamental')?.setValue(''); // Reseta o campo
    }
  }
  ...
  getLastJob() {
    ...
    this.LastJobService.getLastJob().subscribe(
      (response: any) => {

        if (response.result) {
          this.tableListLastJob = response.result;
        } else {
          console.error(
  ...
  LastJobForm: FormGroup;

  get dadosForm() {
    return this, this.LastJobForm.controls;
  }

  enviar() {
    const { id, ...LastJobData } = {
      id: this.LastJobForm.value.id,
      ultimoEmprego: this.LastJobForm.value.ultimoEmprego,
      endereco: this.LastJobForm.value.endereco,
      cidade: this.LastJobForm.value.cidade,
      telefone: this.LastJobForm.value.telefone,
      celular: this.LastJobForm.value.celular,
      funcao: this.LastJobForm.value.funcao,
      tempoDeTrabalho: this.LastJobForm.value.tempoDeTrabalho,
      assinouCarteira: this.LastJobForm.value.assinouCarteira,
      dataEntrada: this.LastJobForm.value.dataEntrada,
      dataSaida: this.LastJobForm.value.dataSaida,
      motivoSaida: this.LastJobForm.value.motivoSaida,
      curriculoId: Number(this.idCurriculo),

      // curriculo: this.LastJobForm.value.casaPropria,
    };

    const obj = this.itemEdicao ? { id, ...LastJobData } : LastJobData;

    if (this.itemEdicao) {
      this.LastJobService.UpdateLastJob(obj).subscribe(
        () => {
      ...
      this.LastJobService.registerLastJob(obj).subscribe(
        () => {
          this.snackBar.open('Registration completed successfully.', 'Close', {
            duration: 2000,
          });

          this.router.navigate(['/infoImportante', this.idCurriculo]);
  ...
  itemEdicao: LastJob;
  edicao(id: number) {
    this.LastJobService.obterLastJob(id).subscribe(
      (response: LastJob) => {
        if (response) {
  ...
  onDeleteLastJob(id: number) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      width: '250px',
      data: 'Are you sure you want to delete this LastJob?',
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result == true) {
        this.LastJobService.deleteLastJob(id).subscribe(
          () => {
